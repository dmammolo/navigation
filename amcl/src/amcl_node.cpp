  ...
  if(resampled || force_publication)
  {
    ...
    if(max_weight > 0.0)
    {
      ...
      if (tf_broadcast_ == true)
      {
        ...
        // Just a QUICKFIX!!
        ros::Duration offset_1;
        offset_1.fromSec(0.02);
        ros::Time transform_expiration1 = (laser_scan->header.stamp +
                                          transform_tolerance_ + offset_1);
        tf::StampedTransform tmp_tf_stamped1(latest_tf_.inverse(),
                                            transform_expiration1,
                                            global_frame_id_, odom_frame_id_);
        this->tfb_->sendTransform(tmp_tf_stamped1);

        ros::Duration offset_2;
        offset_2.fromSec(0.04);
        ros::Time transform_expiration2 = (laser_scan->header.stamp +
                                          transform_tolerance_ + offset_2);
        tf::StampedTransform tmp_tf_stamped2(latest_tf_.inverse(),
                                            transform_expiration2,
                                            global_frame_id_, odom_frame_id_);
        this->tfb_->sendTransform(tmp_tf_stamped2);

        ros::Duration offset_3;
        offset_3.fromSec(0.06);
        ros::Time transform_expiration3 = (laser_scan->header.stamp +
                                          transform_tolerance_ + offset_3);
        tf::StampedTransform tmp_tf_stamped3(latest_tf_.inverse(),
                                            transform_expiration3,
                                            global_frame_id_, odom_frame_id_);
        this->tfb_->sendTransform(tmp_tf_stamped3);
